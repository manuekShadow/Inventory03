...
public interface IInventory {
	...
	public static String SP_FN_INSERTORDER = "{call dba.INVMOV_SP_FN_INSERTINFOOAUTH('?','?',?,?,'?',?,?,'?',?,?,?,'?','?','?',?,'?',?,'?')}";
	
	public static String INSERT_ORDER = "insert into dba.boa_conteos_nube values ('?','?',?,?,?,'?','?',?,'?','?','?',?,?,?,?,?,'?');";

	public static String CONSULT_FOLIOORDER = "{call dba.INVMOV_SP_FN_CONSULTFOLIO('?','?')}";
